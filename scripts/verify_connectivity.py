...
async def main() -> int:
    ...
    try:
        ...
        if abs(offset) > 1000:
            record(
                "Clock skew",
                FAIL,
                f"offset {offset} ms — install/repair NTP; "
        ...
        # -- 2. universe ---------------------------------------------------- #
        tradable = [s for s in client.symbols.values() if s.is_tradable and s.quote_asset == "USDT"]
        if len(tradable) < 50:
        ...
        if not client.authenticated:
            ...
            try:
                ...
                record(
                    "Account access",
                    PASS,
                    f"equity {account.equity:.2f} USDT, available {account.available_balance:.2f}",
                )
            except Exception as exc:  # noqa: BLE001
            ...
            try:
                ...
                if positions:
                    print(
                        "           NOTE: this account already holds positions. "
    ...
    print(
        f"{len(results)} checks: {len(results) - len(failures) - len(warnings)} pass, "
...
async def check_withdrawal_permission(settings: Settings) -> None:
    ...
    try:
        async with (
            ...
            session.get(url, headers={"X-MBX-APIKEY": settings.binance_api_key}) as response,
        ):
    ...
    if not ip_restricted:
        record(
            "IP restriction", WARN, "the key accepts requests from any IP. Restrict it to this VPS."
        )
    else:
...
async def check_market_stream(settings: Settings, symbol: str) -> None:
    ...
    stream = MarketStream(
        settings.ws_url, on_candle, timeframes=("1m",), include_book=False, include_mark=False
    )
    ...
    if received:
        record("WebSocket market stream", PASS, f"{len(received)} kline messages for {symbol}")
    else:
        record(
...
async def check_user_stream(client: BinanceFuturesREST) -> None:
    try:
        ...
        record(
            ...
            PASS,
            f"listen key obtained ({len(key)} chars), keepalive {'ok' if ok else 'FAILED'}",
        )
